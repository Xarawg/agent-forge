...
Новичку — начните с [ONBOARDING.ru.md](ONBOARDING.ru.md) (30 минут от клона до первого прогона). Выбираете инструмент для LLM-разработки — читайте [docs/GUIDE_LLM.ru.md](docs/GUIDE_LLM.ru.md): теория, open-source альтернативы и честные tradeoff'ы.
...
| `forge map [--target DIR]` | Карта сущностей проекта: детерминированный AST-скан → `canon/entities.json` (агентам: анти-дубль каталог + соседи по импортам) и `docs/ENTITIES.md` (людям). $0, без LLM |
