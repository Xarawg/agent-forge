...
- **Онбординг для проекта в любом состоянии** — от пустой папки до легаси-монорепы:
  ...
  - `forge wizard --prompt "..."` — сканирует репо, прогоняет baseline, задаёт уточняющие вопросы при неоднозначности запроса (протокол QUESTIONS) и собирает полный `tasks.wizard.yaml`: задачи, scope, acceptance, бюджеты, гейты — плюс прогноз стоимости. Черновик подтверждает человек (гейт №1), затем запускает.
...
- **Предполётные проверки**: `forge run --dry-run` (прогноз стоимости без запуска) и `forge lint <tasks.yaml>` (валидация контракта + советы по заморозке acceptance — до того, как потрачен первый цент).
...
| `forge lint <tasks.yaml>` | Предполётная проверка: ошибки контракта + советы по заморозке acceptance |
