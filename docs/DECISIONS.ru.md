...
## AF-19. Стратегия контекста: детерминированная карта сущностей вместо эмбеддингов (вариант B)

**Решение (владелец, 2026-08-24):** контекст coder'а собирается структурно, без RAG/эмбеддингов: `forge map` (AST-скан, $0) производит `canon/entities.json` + `docs/ENTITIES.md`; промпт задачи несёт (1) выдержку AGENTS.md целевого репо, (2) анти-дубль каталог имён всех публичных сущностей, (3) сигнатуры соседей scope по графу импортов (глубина 1). Обоснование: эмбеддинг-ретрив недетерминирован и ломает воспроизводимость прогонов; семантическая близость ≠ связь по контракту; структурные карты — лучшая практика 2026 (repo-map Aider, следование по импортам Claude Code). Ручной `canon_snapshot` остаётся опциональным слоем поверх. Поддержка TS/Go отложена до появления tree-sitter.
