...
- **`forge wizard [--prompt "..." | --prompt-file F | --recipe NAME] [--profile ...] [--yes] [--out FILE] [--no-check] [--provider P]`** (`forge/wizard.py`) — черновик полной настройки из промпта своими словами: скан репо → baseline → planner чертит задачи со scope, acceptance (из найденных проверок), бюджетами профиля и гейтами → прогноз стоимости → `tasks.wizard.yaml`. При неоднозначном запросе planner возвращает блок `QUESTIONS:`, и wizard спрашивает интерактивно (`--yes` — дефолты, неинтерактивно). Scope вида `dir/` нормализуется в `dir/**`. Acceptance, который ещё не может пройти (тестовая команда раньше задачи с тестами в репо без тестов), убирается из черновика с явным предупреждением — pytest без тестов падает с exit 5, это гарантированный DISPUTE.
...
- **`forge lint <tasks.yaml>`** (`forge/lint.py`) — предполётная проверка без запуска: ошибки контракта (схема, циклы DAG, висячие зависимости) + советы: заморозка acceptance (тесты вне scope coder'а) и **порядок acceptance** — тестовая команда (pytest/npm test/…) обязана проходить на позиции задачи в DAG: тесты либо уже есть в репо, либо их пишет эта или предшествующая задача (эвристика `has_existing_tests`).
