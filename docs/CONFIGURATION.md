...
- **`forge lint <tasks.yaml>`** (`forge/lint.py`) — pre-flight validation without running: contract errors (schema, DAG cycles, dangling deps) + advisor warnings: frozen acceptance (tests outside coder scope) and **acceptance order** — a test command (pytest/npm test/…) must be passable at the task's position in the DAG, i.e. tests must already exist in the repo or be written by that or an earlier task (`has_existing_tests` heuristic).
